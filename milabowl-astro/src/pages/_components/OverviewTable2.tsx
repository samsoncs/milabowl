...
const OverviewTable2 = ({ data, avatars }: OverviewTable2Props) => {
  const { sortedData, sortOrder, sortProp, handleSort } =
    useSortedTable<GameWeekResult>(data, 'milaRank', 'asc');
  return (
    <div className="text-sm">
      <div className="flex gap-3 pb-2 pl-2 font-bold">
        ...
        <div className="flex gap-1">
          Total
          <SortToggle
            sortDirection={sortProp === 'milaRank' ? sortOrder : undefined}
            onClick={() => handleSort('milaRank')}
          />
        </div>
        <div className="w-4" />
      </div>
      {sortedData.map((result) => {
        ...
        return (
          <div
            key={result.teamName}
            className="flex gap-2 border-b border-slate-200 p-2 dark:border-slate-700"
          >
            <div className="w-4">
          ...
          // <div key={result.gameWeek} className={`flex gap-2 px-2 m-2 rounded-full border-b border-slate-200 dark:border-slate-700 rank-${result.milaRank}`}>
          ...
          //       <span>{result.gwScore}</span>
          //       <span>{result.cumulativeMilaPoints}</span>
          //     </div>
          //   </div>
